...
Il server MCP non è stato configurato in fase di init. Può essere aggiunto in seguito
rieseguendo il Passo 4 della skill `init-project-system`: creerà `.mcp.json` e la cartella
`mcp/` in radice istanziando il template opzionale `.claude/templates/mcp.json`.
